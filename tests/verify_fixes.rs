...
async fn test_empty_include_yields_empty_result() {
    ...
    assert!(result.files.is_empty(), "Result should be empty when include patterns match nothing, found: {:?}", result.files);
}
...
async fn test_security_scan_integration() {
    ...
    assert!(!result.files.iter().any(|f| f.relative_path == "secret_config.txt"), "Sensitive file should not be in content");
    
    // Should be in suspicious files
    match &result.suspicious_files {
        Some(files) => {
            assert!(files.contains(&"secret_config.txt".to_string()), "Sensitive file should be marked suspicious");
        },
